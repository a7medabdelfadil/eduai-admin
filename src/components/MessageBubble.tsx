  ...
  return (
    <>
     <div className="flex items-center space-x-2 rtl:space-x-reverse">
        <img className={`${!isSentByCurrentUser ? "hidden" : ""} w-8 h-8 rounded-full`} src="/images/man.png" alt="User avatar" />
      ...
      <div
        ...
        className={`mb-4 flex w-[320px] break-words rounded-lg p-3 font-semibold ${isSentByCurrentUser
          ? "mr-auto text-left text-white"
          : "ml-auto text-right text-black"
          }`}
      >
        <div
          className={`flex flex-col w-full  max-w-[320px] leading-1.5 p-4 border-gray-200 ${isSentByCurrentUser ? "bg-bgPrimary" : "bg-[#1565C0]"
            } rounded-t-xl rounded-es-xl ${isSentByCurrentUser ? "dark:bg-gray-700" : "dark:bg-blue-900"
            }`}
        >
          <p className={` ${isSentByCurrentUser ? "text-gray-900 dark:text-white" : "text-white dark:text-white"} text-sm font-normal py-2.5 `}>
            {message.content}
          </p>
          <span className={`${isSentByCurrentUser ? "text-secondary":"text-white"} text-sm font-normal `} dir="ltr">
                  ...
                  {!imageError ? (
                    <>
                      <img
                        ...
                        alt="Attachment"
                        className={`w-full max-h-60 rounded-lg object-contain cursor-pointer ${!imageLoaded ? "hidden" : ""
                          }`}
