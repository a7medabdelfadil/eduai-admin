...
const Infrastructure = () => {
  ...
  const [uploadEvent, { isLoading: isUploading }] =
  ...
  const placeholderCards = Array(6)
    .fill(0)
    .map((_, index) => (
      <div key={`placeholder-${index}`} className="animate-pulse">
        <div className="flex h-[160px] w-full overflow-hidden rounded-xl bg-bgPrimary shadow-sm">
          <div className="flex h-full w-full flex-col p-6">
            <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-full bg-bgSecondary"></div>
              <div className="h-4 w-32 rounded bg-bgSecondary"></div>
            </div>
            <div className="mt-4 flex justify-between">
              <div className="h-3 w-20 rounded bg-bgSecondary"></div>
              <div className="h-3 w-16 rounded bg-bgSecondary"></div>
            </div>
            <div className="mt-2 flex justify-between">
              <div className="h-3 w-24 rounded bg-bgSecondary"></div>
              <div className="h-3 w-12 rounded bg-bgSecondary"></div>
            </div>
            <div className="mt-auto h-8 w-full rounded-lg bg-bgSecondary"></div>
          </div>
        </div>
      </div>
    ));
      ...
      >
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              href="/infrastructure/classes/add-class"
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {data?.data.content
            ...
            .map((classItem: Class, index: number) => (
              <Link
                href={`/infrastructure/classes/class-details/${classItem.roomId}`}
              ...
              >
                <div className="flex h-full flex-col overflow-hidden rounded-xl bg-bgPrimary shadow-md transition-shadow hover:shadow-lg">
                  <div className="flex items-center gap-4 border-b p-6">
                    ...
                    <div>
                      <h3 className="font-semibold text-textPrimary group-hover:text-primary">
                        {classItem.classroomName}
                      </h3>
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    <div className="mb-4 space-y-3">
                      ...
                      <div className="h-2 w-full overflow-hidden rounded-full bg-bgSecondary">
                        <div
                          className="h-full bg-primary"
                          style={{
                            width: `${Math.min(100, (classItem.maxCapacity / 50) * 100)}%`,
                          }}
                        ></div>
                      </div>
                    </div>

                    <button className="mt-auto inline-flex w-full items-center justify-center gap-2 rounded-lg bg-bgSecondary py-2.5 text-sm font-medium transition-colors group-hover:bg-primary group-hover:text-white">
                      <BookOpen size={16} />
                      <span>
                        {currentLanguage === "ar"
                          ? "عرض التفاصيل"
        ...
        <Modal isOpen={isModalOpen} onClose={handleCloseModal}>
          <div className="p-1">
            ...
            <form onSubmit={handleUploadEvent} className="space-y-4">
              <div
                className="border-primary/30 bg-primary/5 hover:border-primary/50 hover:bg-primary/10 cursor-pointer rounded-lg border-2 border-dashed p-6 text-center transition-colors"
                onDragOver={e => e.preventDefault()}
                onDrop={handleDrop}
              >
                {!file ? (
                  <div>
                    <div className="bg-primary/10 mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full text-primary">
                      <Upload size={20} />
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="bg-primary/20 flex h-10 w-10 items-center justify-center rounded-full text-primary">
                          <svg
                            className="h-5 w-5"
                            viewBox="0 0 24 24"
                            fill="currentColor"
