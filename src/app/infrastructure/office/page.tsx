...
const Office = () => {
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      toast.error(t("Failed to delete", "فشل الحذف", "Échec de la suppression"));
    }
  };
        ...
        <div className="bg-bgPrimary rounded-xl">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 px-4 py-4 rounded-lg">
            ...
            <Link
              ...
              className="rounded-xl bg-primary px-4 py-2 text-white font-semibold hover:bg-secondary"
            >
          ...
          <div className="relative overflow-auto shadow-md sm:rounded-lg bg-bgPrimary">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t("Lab Name", "اسم المختبر", "Nom du laboratoire")}</TableHead>
                  <TableHead>{t("Building Number", "رقم المبنى", "Numéro du bâtiment")}</TableHead>
                  <TableHead>{t("Category", "الفئة", "Catégorie")}</TableHead>
                  <TableHead>{t("Room Number", "رقم الغرفة", "Numéro de la salle")}</TableHead>
                  <TableHead>{t("Max Capacity", "الطاقة الاستيعابية القصوى", "Capacité maximale")}</TableHead>
                  <TableHead>{t("Floor Number", "رقم الطابق", "Numéro de l'étage")}</TableHead>
