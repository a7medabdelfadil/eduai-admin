...
import { useCreateBusCostMutation } from "@/features/Financial/transportApi";
...
const AddTransport = () => {
  ...
  const { data: regionsData, isLoading: regionsLoading } = useGetAllReginionIDQuery(null);
  const [createBusCost, { isLoading: submitting }] = useCreateBusCostMutation();
      ...
      <Container>
        <div className="-ml-1 -mt-2 mb-8 flex items-center justify-between">
          <h1 className="text-3xl font-semibold">
            {currentLanguage === "en" ? "Add Transport" : currentLanguage === "ar" ? "إضافة نقل" : "Ajouter un transport"}
          </h1>
        </div>
        <form className="flex h-full w-full items-center justify-center" onSubmit={handleSubmit(onSubmit)}>
          <div className="w-[90] rounded-xl bg-bgPrimary p-10 md:w-[80%]">
            ...
            <div className="grid grid-cols-2 gap-4 p-6 max-[1278px]:grid-cols-1">
              <label htmlFor="regionId" className="grid text-[18px] font-semibold">
                {currentLanguage === "ar" ? "المنطقة" : currentLanguage === "fr" ? "Région" : "Region"}
                <select id="regionId" {...register("regionId")} className="w-full rounded-xl border border-borderPrimary bg-bgPrimary px-4 py-3 text-textPrimary outline-none max-[471px]:w-[350px]">
                  <option value="">
                    {currentLanguage === "ar" ? "اختر المنطقة" : currentLanguage === "fr" ? "Sélectionnez une région" : "Select Region"}
                  </option>
                  {regionsData?.data && regionsData.data.map((region: any) => (
